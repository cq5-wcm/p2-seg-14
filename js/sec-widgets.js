...
CQ.security.PermissionsDetailPanel = CQ.Ext.extend(CQ.Ext.Panel, {
    ...
    renderAuthorizableId: function(value, metaData, record, rowIndex, colIndex, store){
        return CQ.shared.XSS.geXSSValue(record.get('authorizableId'));
    }
});
...
CQ.security.data.UserAclStore = CQ.Ext.extend(CQ.security.data.AclStore,{
    ...
    requestData:function(contentPath) {
        ...
        //for details refer to NPR-6883
        var panelId=$(".cq-siteadmin-tabpanel .x-tab-strip-wrap .x-tab-strip-active").attr("id");
        //checks if the panel opened is the search panel
        if(panelId && (panelId.indexOf("cq-siteadmin-tabpanel__cq-siteadminsearchpanel")!=-1 || panelId.indexOf("cq-damadmin-tabpanel__cq-siteadminsearchpanel")!=-1 )){
            //send request for current path only
            var url = CQ.HTTP.addParameter(this.url, "path", contentPath);
	        url = CQ.HTTP.addParameter(url, "currentNodeOnly", "true");
        } else {
            //if it is not the search panel,no change in behaviour
...
CQ.security.AuthRelationPanel = CQ.Ext.extend(CQ.security.UserAdminGridPanel , {
    ...
    saveHandler:function() {
        ...
        CQ.HTTP.multiPartPost(url, function(request) {
...
CQ.security.UserProperties = CQ.Ext.extend(CQ.Ext.Panel, {
    ...
    constructor: function(config) {
        ...
        this.userForm = new CQ.Ext.form.FormPanel({
            ...
            "items":[{
                    ...
                    "xtype":"checkbox",
                    "fieldLabel":CQ.I18n.getMessage("Is a Lead"),
                    "anchor":"100%",
                    "name":"isLead"
    ...
    loadRecord: function(rec) {
        ...
        if(type == "user"){
            //suspending events, as "save" user button will get enabled
            this.activeForm.getForm().findField("isLead").suspendEvents(false);
            if(rec.data["cq:authorizableCategory"] != "mcm"){
				this.activeForm.getForm().findField("isLead").setValue("false");
            }else{
                this.activeForm.getForm().findField("isLead").setValue("true");
            }
			this.activeForm.getForm().findField("isLead").resumeEvents();
            //resuming events
        }
    ...
    setPassword : function() {
        var dialogCfg = {
            ...
            "formUrl":CQ.security.UserProperties.getUrl(this.getForm(), true),
            "params": {
                "_charset_":"utf-8"
            },
            "items": {
                ...
                "items": {
                    ...
                    "currentPassword": {
                        "inputType":"password",
                        "fieldLabel":CQ.I18n.getMessage("Your Password"),
                        "fieldDescription":CQ.I18n.getMessage("Please enter your own password"),
...
CQ.security.UserProperties.getUrl = function(frm, useGraniteServlet){
    ...
    constructor: function(config) {
        ...
        this.fieldCfgs = [{
            ...
            "options": [
                ...
                {"value":"pt-br","text":CQ.I18n.getMessage("Portuguese (Brazil)")},
                {"value":"zh-cn","text":CQ.I18n.getMessage("Chinese")},
                {"value":"zh-tw","text":CQ.I18n.getMessage("Chinese (Taiwan)")},
                {"value":"ja","text":CQ.I18n.getMessage("Japanese")},
                {"value":"ko-kr","text":CQ.I18n.getMessage("Korean")}
            ],
            ...
            "listeners":{
                ...
                'activate':{
                    "fn":function(panel) {
                        ...
                        var rec = this.selectionStore.getAt(0);
                        if (!rec) return;
                        
                        if (rec.get("_isMembersLoaded")) return;
                        
                        var mask = new CQ.Ext.LoadMask(panel.bwrap);
                        mask.show();
                        
                        CQ.Ext.Ajax.request({
                            url: rec.data.home + ".userprops.json",
                            method: "GET",
                            success: function(response) {
                                var props = CQ.Ext.decode(response.responseText);
                                rec.set("members", CQ.security.data.AuthRecord.arrayConverter(props.members));
                                rec.set("_isMembersLoaded", true);
                                panel.onSelectionChanged(this.selectionStore, rec);
                                panel.activationHandler(panel.getSelectionStore());
                                mask.hide();
                                mask.destroy();
                            },
                            scope: this
